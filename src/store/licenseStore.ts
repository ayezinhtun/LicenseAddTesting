...
const ATTACHMENTS_BUCKET =
  (import.meta as any).env?.VITE_SUPABASE_ATTACHMENTS_BUCKET || "attachments";
...
interface LicenseState {
  ...
  addLicense: (
    license: Omit<License, "id" | "created_at" | "updated_at">,
  ) => Promise<License>;
  ...
  getSerialsNearExpiry: (
    days: number,
  ) => Promise<Array<{ license: License; serial: LicenseSerial }>>;
  ...
  validateLicense: (
    license: Partial<License> & { serials?: LicenseSerial[] },
...
export const useLicenseStore = create<LicenseState>((set, get) => ({
  ...
  validateLicense: (license) => {
    ...
    if (serials.length === 0) {
      ...
      serials.forEach((s, idx) => {
        if (!s.serial_or_contract?.trim())
          errors.push(`Serial row ${idx + 1}: Serial/Contract No. is required`);

        if (!s.start_date)
          errors.push(`Serial row ${idx + 1}: Start Date is required`);

        if (!s.end_date)
          errors.push(`Serial row ${idx + 1}: End Date is required`);
        ...
        if (
          ...
          errors.push(
            `Serial row ${idx + 1}: End Date must be after Start Date`,
          );
        }

        if (s.qty === undefined || s.qty < 1)
          errors.push(`Serial row ${idx + 1}: Qty must be at least 1`);

        if (s.unit_price === undefined || s.unit_price < 0)
          errors.push(`Serial row ${idx + 1}: Unit Price must be >= 0`);

        if (s.currency !== "MMK" && s.currency !== "USD")
          errors.push(`Serial row ${idx + 1}: Currency must be MMK or USD`);
      });
    }
    ...
    if (dupes.length > 0) {
      errors.push(
        `Duplicate Serial/Contract not allowed: ${Array.from(new Set(dupes)).join(", ")}`,
      );
    }
  ...
  fetchLicenses: async (page = 1) => {
    ...
    try {
      ...
      if (filters.search) {
        query = query.or(
          `item.ilike.%${filters.search}%,vendor.ilike.%${filters.search}%,project_name.ilike.%${filters.search}%`,
        );
      }
      ...
      if (serialFilterRaw && String(serialFilterRaw).trim()) {
        ...
        if (!smErr) {
          const licenseIds = Array.from(
            new Set((serialMatches || []).map((r) => r.license_id)),
          );
  ...
  addLicense: async (licenseData) => {
    try {
      ...
      const requestedAssign = (licenseData as any)?.project_assign as
        | string
        | undefined;
      ...
      if (user && (user.role === "super_user" || user.role === "user")) {
        ...
        if (!assignments || !assignments.includes(requestedAssign as any)) {
          throw new Error(
            `You are not assigned to project ${requestedAssign}. Allowed: ${assignments?.join(", ") || "none"}`,
          );
        }
      }
      ...
      const distributors: LicenseDistributor[] =
        distributorsRaw as LicenseDistributor[];

      // Validate license data according to new rules

      const validation = get().validateLicense({
        ...(licenseData as any),
        serials,
      });
      ...
      if (error) {
        ...
        if (code === "23505" && message.includes("serial_number")) {
          throw new Error(
            "Serial number already exists. Please enter a unique serial/contract number.",
          );
        }
    ...
    try {
      ...
      if (error) {
        ...
        if (code === "23505" && message.includes("serial_number")) {
          ...
          throw new Error(
            `Serial number "${sn}" already exists on another license.`,
          );
        }
      ...
      const serialsIncoming = (updates as any).serials as
        | LicenseSerial[]
        | undefined;
      ...
      if (serialsIncoming) {
        ...
        const existingIds = new Set(
          (existingSerials || []).map((r: any) => r.id),
        );

        const incomingIds = new Set(
          serialsIncoming.filter((s) => s.id).map((s) => s.id as string),
        );
        ...
        const toUpdate = serialsIncoming.filter(
        ...
        // Apply updates (ONLY serial_or_contract; do NOT touch dates/cost/etc.)
        ...
        for (const s of toUpdate) {
          ...
          if (upErr) {
            if ((upErr as any).code === "23505") {
              throw new Error(
                `Serial/Contract "${s.serial_or_contract}" already exists. It must be unique.`,
              );
            }
        ...
        if (toInsert.length > 0) {
          for (const s of toInsert) {
            if (
              ...
              throw new Error(
                "Serial row is missing required fields (serial, dates, qty, unit price, currency).",
              );
            }
          }
          ...
          if (insErr) {
            if ((insErr as any).code === "23505") {
              ...
              throw new Error(
                `Serial/Contract "${first}" already exists. It must be unique.`,
              );
            }
      ...
      const customersIncoming = (updates as any).customers as
        | LicenseCustomer[]
        | undefined;

      if (customersIncoming) {
        const { data: existingCustomers, error: exErr } = await supabase
        ...
        const existingIds = new Set(
          (existingCustomers || []).map((r: any) => r.id),
        );

        const incomingIds = new Set(
          customersIncoming.filter((c) => c.id).map((c) => c.id as string),
        );

        const toInsert = customersIncoming.filter((c) => !c.id);

        const toUpdate = customersIncoming.filter(
      ...
      const distributorsIncoming = (updates as any).distributors as
        | LicenseDistributor[]
        | undefined;

      if (distributorsIncoming) {
        const { data: existingDistributors, error: exErr } = await supabase
        ...
        const existingIds = new Set(
          (existingDistributors || []).map((r: any) => r.id),
        );

        const incomingIds = new Set(
          distributorsIncoming.filter((d) => d.id).map((d) => d.id as string),
        );

        const toInsert = distributorsIncoming.filter((d) => !d.id);

        const toUpdate = distributorsIncoming.filter(
  ...
  setFilters: (filters) => {
    ...
    if ("vendor" in filters) {
      ...
      next.vendor =
        !v || String(v).trim().toLowerCase() === "all" ? "" : String(v).trim();
    }
    ...
    if ("serial" in filters) {
      ...
      next.serial =
        !s || String(s).trim().toLowerCase() === "all" ? "" : String(s).trim();
    }
    ...
    if ("serial_number" in filters) {
      ...
      next.serial_number =
        !s2 || String(s2).trim().toLowerCase() === "all"
          ? ""
          : String(s2).trim();
    }
  ...
  renewLicense: async (id, payload) => {
    try {
      ...
      if (payload.newEndDate && parseISO(payload.newEndDate) > new Date()) {
        await supabase
          .from("license_serials")

          .update({ renewal: false, last_notified_on: null })
    ...
    try {
      ...
      if (serialsIncoming) {
        ...
        for (const s of toUpdate) {
          ...
          if (typeof s.serial_or_contract === "string")
            payload.serial_or_contract = s.serial_or_contract;

          if (typeof s.start_date === "string")
            payload.start_date = s.start_date; // YYYY-MM-DD

          if (typeof s.end_date === "string") payload.end_date = s.end_date; // YYYY-MM-DD

          if (typeof s.qty === "number") payload.qty = s.qty;

          if (typeof s.unit_price === "number")
            payload.unit_price = s.unit_price;

          if (typeof s.currency === "string") payload.currency = s.currency;

          if ("po_no" in s) payload.po_no = s.po_no ?? null;

          if (typeof s.notify_before_days === "number")
            payload.notify_before_days = s.notify_before_days;
  ...
  searchLicenses: async (query) => {
    try {
      const { data, error } = await supabase
        ...
        .or(
          `item.ilike.%${query}%,vendor.ilike.%${query}%,project_name.ilike.%${query}%,customer_name.ilike.%${query}%`,
        )
  ...
  getVendorStats: () => {
    ...
    activeLicenses.forEach((license) => {
      const existing = vendorMap.get(license.vendor) || {
        count: 0,
        totalCost: 0,
      };
  ...
  getProjectStats: () => {
    ...
    activeLicenses.forEach((license) => {
      const existing = projectMap.get(license.project_name) || {
        count: 0,
        totalCost: 0,
      };
  ...
  getNearSerialExpiryCount: async (days) => {
    try {
      ...
      const { count, error } = await supabase
        ...
        .select("id, licenses!inner(project_assign)", {
          count: "exact",
          head: true,
        })
