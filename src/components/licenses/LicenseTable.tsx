...
export const LicenseTable: React.FC<LicenseTableProps> = ({
  ...
  const isAllSelected =
    licenses.length > 0 && selectedLicenses.length === licenses.length;
  const isIndeterminate =
    selectedLicenses.length > 0 && selectedLicenses.length < licenses.length;
                      ...
                      {(license as any).license_serials?.length ? (
                        <div className="flex flex-wrap gap-1 mt-1">
                          {(license as any).license_serials.map((s: any) => (
                            ...
                            >
                              {s.serial_or_contract} - Start:{" "}
                              {format(parseISO(s.start_date), "dd MMM yyyy")} →
                              End: {format(parseISO(s.end_date), "dd MMM yyyy")}
                            </span>
                          ))}
                        </div>
                      ) : null}
