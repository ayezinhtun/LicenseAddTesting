...
export const RecentlyDeleted: React.FC = () => {
   ...
   const load = async () => {
    ...
    const handlePermanentDelete = async (id: string) => {
        if (!window.confirm('Permanently delete this license? This cannot be undone.')) return;
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-gray-600 mt-1">Items deleted within the last 30 days</p>
                </div>
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {rows.map(l => (
                                    <tr key={l.id} className="hover:bg-gray-50 transition-colors">
                                        <td className="px-6 py-4 text-sm text-gray-900">{l.project_name}</td>
                                        <td className="px-6 py-4 text-sm text-gray-900">{l.vendor}</td>
                                        <td className="px-6 py-4 text-sm text-gray-900">{l.item_description}</td>
                                        <td className="px-6 py-4 text-sm text-gray-700">
                                            {(l as any).deleted_at ? new Date((l as any).deleted_at).toLocaleString() : '-'}
                                        </td>
