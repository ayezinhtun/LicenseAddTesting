  ...
  const fetchProfiles = async () => {
    ...
    try {
      ...
      const merged = (profiles as any[]).map((p) => {
        ...
        const roleNorm: Role = (
          allowedRoles.includes(roleRaw as Role) ? roleRaw : "user"
        ) as Role;
        const statusNorm: Status = (
          allowedStatus.includes(statusRaw as Status) ? statusRaw : "pending"
        ) as Status;
        return {
          ...p,
          role: roleNorm,
      ...
      console.error(err);
      toast.error(
        "Failed to load users. Ensure table user_profiles exists and RLS allows admin read.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const toggleAssign = async (row: ProfileRow, assign: string) => {
    ...
    try {
      ...
      if (hasIt) {
        ...
        if (error) throw error;
        patchRow(row.id, {
          assignments: row.assignments.filter((a) => a !== assign),
        });
      } else {
        const { error } = await supabase
          .from("user_project_assigns")
      ...
      >
        <div>
          ...
          <p className="text-gray-600 mt-1">
            Manage roles, project assignment and approval status
          </p>
        </div>
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm">
                      <Select
                        value={u.role}
                        onChange={(value) =>
                          patchRow(u.id, { role: value as Role })
                        }
                        options={roleOptions}
                        className="w-40"
                      />
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm">
                      <Select
                        value={u.status}
                        onChange={(value) =>
                          patchRow(u.id, { status: value as Status })
                        }
