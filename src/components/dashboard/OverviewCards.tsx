...
export const OverviewCards: React.FC = () => {
  const { licenses, getNearSerialExpiryCount, getExpiredSerialsCount } =
    useLicenseStore();
  ...
  const uniqueProjects = new Set(
    licenses.map((license) => license.project_name),
  ).size;
  const uniqueCustomers = new Set(
    licenses.map((license) => license.customer_name),
  ).size;
              ...
              <div className="flex-1">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span
                      className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${getChangeColor(card.changeType)}`}
                    >
