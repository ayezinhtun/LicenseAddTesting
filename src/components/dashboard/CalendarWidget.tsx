...
export const CalendarWidget: React.FC = () => {
  ...
  const [upcomingSerials, setUpcomingSerials] = useState<Array<{ license: any; serial: any }>>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!mounted) return;
      const sorted = [...rows]
        .sort((a, b) => parseISO(a.serial.end_date).getTime() - parseISO(b.serial.end_date).getTime())
        .slice(0, 5);
  ...
  const getDaysUntilExpiry = (dateString: string) => differenceInDays(parseISO(dateString), new Date());
  const getUrgencyColor = (days: number) => {
    if (days <= 0) return 'text-red-600 bg-red-50 border-red-200';
    if (days <= 7) return 'text-red-600 bg-red-50 border-red-200';
    if (days <= 30) return 'text-orange-600 bg-orange-50 border-orange-200';
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-200">
      ...
      <div className="p-4">
        {upcomingSerials.length === 0 ? (
          <div className="text-center text-gray-500 py-8">
            <Calendar className="h-12 w-12 mx-auto mb-3 text-gray-300" />
            <h3 className="text-sm font-medium text-gray-900 mb-1">No upcoming serial expirations</h3>
          ...
          <div className="space-y-3">
            {upcomingSerials.map(({ license, serial }) => {
              ...
              return (
                <div key={serial.id} className="flex items-center justify-between p-4 rounded-lg border border-gray-100 hover:bg-gray-50 transition-colors duration-150">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium text-gray-900 truncate">
                      ...
                      <div className={`ml-3 px-2 py-1 rounded-full text-xs font-medium border ${urgencyColor}`}>
                        <div className="flex items-center space-x-1">
                          <UrgencyIcon className="h-3 w-3" />
                          <span>
                            {daysUntil <= 0 ? 'Expired' : daysUntil === 1 ? '1 day' : `${daysUntil} days`}
                          </span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                      <p className="text-xs text-gray-600 flex items-center">
                        <Clock className="h-3 w-3 mr-1" />
                        Expires: {format(parseISO(serial.end_date), 'MMM dd, yyyy')}
                      </p>
                    </div>
                  </div>
                </div>
